...
export async function removeUserByEmail(email) {
  ...
  const theInfo = await theUsers.findOneAndDelete({
...
export async function sendEmail(email, field, text) {
  ...
  const finalUser = await changeField(
    email,
    field,
    helpers.doubleHash(code.toString())
  );
  return finalUser;
}
...
export async function checkCode(email, code) {
  ...
  let finalUser = await changeField(theUser["email"], "emailVerified", true);
  return finalUser;
}
...
export async function registeredEvents(userId) {
  helpers.checkArgs(arguments, 1);
